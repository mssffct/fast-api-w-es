...
def test_search_definite(text_piece_obj):
    res = es.search(index=text_piece_obj[1], query={
        "match": {"page_number": text_piece_obj[0].page_number}}
                    )
...
def test_search_definite_not_validated_doc():
    response = client.get(
        f"search/definite",
        params={"doc_name": "0", "page_number": 1}
    )
...
def test_search_by_type(text_piece_obj):
    res = es.search(
        index=text_piece_obj[1], query={
            "match": {
                "text_type": text_piece_obj[0].text_type
            }
        }
    )
    assert res["hits"]["total"] > 0
